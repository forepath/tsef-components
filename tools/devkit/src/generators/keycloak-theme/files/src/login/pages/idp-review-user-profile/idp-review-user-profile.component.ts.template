...
import type { UserProfileFormFieldsComponent } from '../../components/user-profile-form-fields/user-profile-form-fields.component';
...
export class IdpReviewUserProfileComponent extends ComponentReference {
  kcContext =
    inject<Extract<KcContext, { pageId: 'idp-review-user-profile.ftl' }>>(
      KC_LOGIN_CONTEXT,
    );
  ...
  socialProvidersNode = viewChild<TemplateRef<HTMLElement>>(
    'socialProvidersNode',
  );

  userProfileFormFields = input<Type<UserProfileFormFieldsComponent>>();
  isFormSubmittable = toSignal(
    this.#userProfileFormService.formState$.pipe(
      map((s) => s.isFormSubmittable),
    ),
    { initialValue: false },
  );
